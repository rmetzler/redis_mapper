...
Entitable is a redis/ruby-based ORM gem.
...
Model.find doesn't actually fetch the record. Model.find! however does and return nil if record doesnt exist.
  ...
  msg4 = Message.find("nonexistentid")
...
Indexes are base on redis sorted sets. Band methods fetch objects, non-bang fetch ids
...
== Configuration
  ...
  require "entitable"
  Entitable.connect(:redis_uri => "localhost:6379", :redis_prefix => "myproject-dev")
